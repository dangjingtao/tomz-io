...
export function HomePage() {
  const recentPosts = loadBlogPosts().slice(0, 4);
  ...
  return (
    <>
      ...
      <div className="wrap home-page">
        ...
        <Section eyebrow="WRITING" title="最近写下" intro="这里会直接读取 Markdown 内容。BR002 不迁旧文章，所以现在保持诚实的空状态。">
          {recentPosts.length ? (
            <div className="post-list">
              {recentPosts.map((post) => (
                <Link className="post-row" to={post.urlPath} key={post.urlPath}>
                  <span>{post.frontmatter.group}</span>
                  <div><h3>{post.frontmatter.title}</h3><p>{post.frontmatter.description}</p></div>
