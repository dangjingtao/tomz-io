...
function staticNavigationHtml(base: string): string {
  return siteNavigation
    .map(({ label, path }) => `<li><a href="${hrefFor(base, path)}">${escapeHtml(label)}</a></li>`)
    .join("");
}

function bookshelfStaticNav(base: string): string {
  return `<nav class="top-nav docs-header seo-static-header"><div class="wrap"><a class="brand" href="${hrefFor(base, "/")}">${siteName}</a><ul class="menu">${staticNavigationHtml(base)}</ul></div></nav>`;
}

function staticHomeBody(base: string) {
  const href = (path: string) => hrefFor(base, path);
  return `<nav class="top-nav seo-static-header"><div class="wrap"><a class="brand" href="${href("/")}">${siteName}</a><ul class="menu">${staticNavigationHtml(base)}</ul></div></nav><main class="seo-static-content home-v1-static"><header class="wrap"><span>INDEPENDENT DEVELOPER / PRODUCT DESIGNER</span><h1>${siteName}</h1><p>${homeIntro}</p></header><section class="wrap"><h2>最近发生的事</h2><p>从公开的项目、写作与生活记录里挑选最能代表当下的近况。</p></section><section class="wrap"><h2>最近写了</h2><p><a href="${href("/blogs")}">查看最近文章 →</a></p></section><section class="wrap"><h2>关于</h2><p>我关注独立开发、产品设计，以及 AI 如何进入真实的工作与生活。</p></section></main>`;
}
